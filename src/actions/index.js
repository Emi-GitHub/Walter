export const deleteEmployee = (employees, x) => {
  return {
    ...
    payload: employees.filter((employee, i) => {
      return i !== x;
    }),
  };
};
...
export const updateDailyComponentValue = (dailys, x, name, time, onTime) => {
  return {
    ...
    payload: dailys.map((el, i) =>
      i === x ? { ...el, name: name, time: time, onTime: onTime } : el
    ),
  };
};
...
export const setDailyMode = (value) => {
  return {
    type: "SET_DAILY_MODE",
    payload: value,
  };
